...
- (void)loadProfilesData
{
    ...
    {
        NSString *profileIdStr = [[NSUserDefaults standardUserDefaults]
                                  stringForKey:kCurrentProfile];
        
        MYSProfile *currentPro5 = [[MYSDataManager shared] getProfileWithId:profileIdStr];
        if (currentPro5)
        {
            int index = [_profileDataSource indexOfObject:currentPro5];
...
{
    int count = [self.profileDataSource count];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    headerView.frame = CGRectMake(0, 0, CGRectGetWidth(self.tbvProfiles.frame), 30.0f);
