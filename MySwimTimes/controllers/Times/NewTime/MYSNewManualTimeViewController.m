...
- (void) _initData
{
    ...
    self.arySwimmers = [arySwimmers sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2)
                          {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(self.viewPopMenu.tag == 0)
    {
        _selectedSwimmerIndex = indexPath.row;
    }
    else if(self.viewPopMenu.tag == 1)
    {
        if(indexPath.row == 0)
        ...
        else
        {
            ...
            _selectedMeetIndex = indexPath.row - 1;
        }
    }
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    else if(textField == self.txtSwimTime || textField == self.txtReactionTime || textField.tag >= 1)
    {
        ...
        {
            _editingTimeIndex = textField.tag;
        }
